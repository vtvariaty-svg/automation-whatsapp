...
export async function GET(request: Request) {
  try {
    ...
    console.log(`[TemplatesCustom][GET] tenantId: ${auth.tenantId}`);
    ...
    console.error(`[TemplatesCustom][GET][ERROR] ${error.message}\n`, error.stack);
    return NextResponse.json({ error: 'Falha ao carregar templates locais. Verifique o banco de dados.' }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    console.log(`[TemplatesCustom][POST] tenantId: ${auth.tenantId}`);
    ...
    console.error(`[TemplatesCustom][POST][ERROR] ${error.message}\n`, error.stack);
    return NextResponse.json({ error: 'Falha ao criar template local. Tente novamente mais tarde.' }, { status: 500 });
  }
}
