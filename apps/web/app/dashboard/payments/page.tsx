    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          {data && (
            <p className="text-sm text-gray-400 mt-0.5">{data.total} cobrança{data.total !== 1 ? "s" : ""}</p>
          )}
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => router.push("/dashboard/payments/config")}
            className="px-3 py-2 rounded-lg border border-white/10 text-gray-300 text-sm hover:bg-white/5 transition-colors"
          >
            ⚙ Configurar Asaas
          </button>
          <button
      ...
      <div className="flex flex-wrap gap-3">
        <select
          ...
          className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-indigo-500"
        >
        ...
        {(statusFilter || dateFrom || dateTo) && (
          <button
            ...
            className="px-3 py-2 rounded-lg border border-white/10 text-gray-400 text-sm hover:text-white hover:bg-white/5 transition-colors"
          >
            Limpar
          </button>
        )}
              ...
              {data.payments.map((p) => (
                <tr key={p.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                  <td className="px-4 py-3">
                    ...
                    <p className="text-gray-500 text-xs">{p.customerPhone}</p>
                  </td>
                  <td className="px-4 py-3 text-white font-medium">{formatCurrency(p.amount)}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLOR[p.status] ?? "bg-gray-500/15 text-gray-400"}`}>
                      {STATUS_LABEL[p.status] ?? p.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-gray-300 text-xs">{BILLING_LABEL[p.billingType] ?? p.billingType}</td>
                  <td className="px-4 py-3 text-gray-300 text-xs">{p.dueDate}</td>
                  ...
                  <td className="px-4 py-3">
                    {p.invoiceUrl ? (
                      <button
                        ...
                        title={p.invoiceUrl}
                        className="px-2.5 py-1 rounded-lg bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-400 text-xs transition-colors"
                      >
                        Copiar link
                      </button>
                    ) : (
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button
            ...
            disabled={page === 1}
            className="px-3 py-1.5 rounded-lg border border-white/10 text-gray-400 text-sm disabled:opacity-40 hover:bg-white/5 transition-colors"
          >
            ← Anterior
          </button>
