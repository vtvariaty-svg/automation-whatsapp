...
const PRIMARY_HREF = '/demo'
const SPECIALIST_WA =
  'https://wa.me/5519995993220?text=Ol%C3%A1%2C+quero+falar+com+um+especialista+da+Variaty.'

// Step icons (SVG paths, used inline)
const STEP_ICONS = [
  // contact / message
...
const BENEFIT_ICONS = [
  // check-circle
  <svg key="0" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
  // users
  <svg key="1" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>,
  // zap
  <svg key="2" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>,
  // clipboard
  <svg key="3" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>,
  // star
...
export function NicheLandingShell({ content }: { content: NicheContent }) {
  ...
  return (
    <main className="bg-white min-h-screen font-sans selection:bg-blue-100 selection:text-blue-900">
      <MetaPixel />
      <UTMPersist />
      <NicheLandingHeader primaryCTA={content.hero.primaryCTA} primaryHref={PRIMARY_HREF} />

      {/* ── HERO ──────────────────────────────────────────────────── */}
      <section
        className="relative pt-28 pb-20 lg:pt-36 lg:pb-28 overflow-hidden bg-[#080d19]"
        data-track="section_view"
        data-section="hero"
      >
        {/* Theme ambient glow */}
        <div
          ...
          style={{
            background: `radial-gradient(ellipse 70% 60% at 50% -5%, ${tc.heroGlow}, transparent)`,
          }}
        />
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          {/* Niche badge */}
          <div
            className={`inline-flex items-center gap-2 ${tc.heroBadgeBg} ${tc.heroBadgeBorder} rounded-full px-4 py-2 mb-8`}
          >
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-extrabold text-white tracking-tight leading-[1.08] mb-6">
            <span
              className="text-transparent bg-clip-text"
              style={{
                backgroundImage: `linear-gradient(135deg, ${tc.gradientFrom}, ${tc.gradientTo})`,
              }}
            >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-8">
            <Link
              href={PRIMARY_HREF}
              className="w-full sm:w-auto flex items-center justify-center gap-2 text-base font-extrabold bg-blue-600 text-white px-8 py-4 rounded-xl shadow-[0_8px_30px_rgba(37,99,235,0.35)] hover:bg-blue-500 transition-all hover:-translate-y-1"
            ...
            <a
              href="#como-funciona"
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-8 py-4 rounded-xl text-slate-300 font-semibold transition-all text-sm"
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2">
            {content.hero.trustBullets.map((bullet, i) => (
              <span key={i} className="flex items-center gap-1.5 text-sm text-slate-400 font-medium">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8">
            {content.hero.trustBullets.map((bullet, i) => (
              <div key={i} className="flex items-center gap-2.5">
                <span className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${tc.problemIconBg}`}>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <ul className="flex flex-col gap-3">
            {content.problem.bullets.map((bullet, i) => (
              <li
                key={i}
                className="flex items-start gap-4 bg-white rounded-2xl px-5 py-4 border border-slate-200/70 shadow-[0_2px_8px_rgba(0,0,0,0.03)]"
              >
                <span className={`mt-0.5 shrink-0 w-6 h-6 rounded-full flex items-center justify-center ${tc.problemIconBg}`}>
                  <svg className={`w-3.5 h-3.5 ${tc.problemIconText}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <section className="py-16 lg:py-20 bg-white">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-[#0f172a] tracking-tight leading-tight mb-5">
          ...
          <Link
            href={PRIMARY_HREF}
            className="inline-flex items-center gap-2 bg-blue-600 text-white font-bold text-sm px-7 py-3.5 rounded-xl hover:bg-blue-700 transition-all hover:-translate-y-0.5 shadow-md hover:shadow-blue-200/50"
            data-track="cta_primary"
          >
            {content.hero.primaryCTA}
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-4">
              {content.howItWorks.steps.map((step, i) => (
                <div key={i} className="flex flex-col items-center text-center group">
                  {/* Icon circle */}
                  <div
                    className={`relative w-[4.5rem] h-[4.5rem] rounded-2xl bg-white border border-slate-100 shadow-md flex items-center justify-center mb-5 ${tc.problemIconText} group-hover:-translate-y-1 transition-all duration-300 z-10`}
                  >
                    {STEP_ICONS[i]}
                    <div className="absolute -top-2.5 -right-2.5 w-6 h-6 bg-[#0f172a] text-white font-extrabold text-xs rounded-full flex items-center justify-center shadow ring-2 ring-white">
                      {i + 1}
                    </div>
                  </div>
                  <h3 className="font-extrabold text-[#0f172a] text-sm sm:text-base mb-2 leading-snug">
                    {step.title}
                  </h3>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {content.benefits.items.map((item, i) => (
              ...
              >
                <span className={`inline-flex w-9 h-9 rounded-xl items-center justify-center mb-4 ${tc.problemIconBg} ${tc.problemIconText} group-hover:scale-110 transition-transform duration-200`}>
                  {BENEFIT_ICONS[i % BENEFIT_ICONS.length]}
                </span>
                <h3 className="font-extrabold text-[#0f172a] text-sm sm:text-base mb-2">
                  {item.title}
                </h3>
                <p className="text-xs sm:text-sm text-slate-500 leading-relaxed font-medium">
                  {item.description}
      ...
      >
        <div
          className="absolute inset-0 pointer-events-none"
          style={{
            background: `radial-gradient(ellipse 60% 60% at 50% 50%, ${tc.heroGlow}, transparent)`,
          }}
        />
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {content.proof.focusPoints.map((point, i) => (
              ...
              >
                ...
                <span className="text-sm sm:text-base text-slate-300 font-medium leading-snug">
                  {point}
                </span>
              </li>
            ))}
          ...
          <div className="mt-10 text-center">
            <Link
              href={PRIMARY_HREF}
              className="inline-flex items-center gap-2 bg-blue-600 text-white font-bold px-8 py-4 rounded-xl hover:bg-blue-500 transition-all hover:-translate-y-1 shadow-[0_8px_30px_rgba(37,99,235,0.3)]"
              data-track="cta_primary"
            >
              {content.hero.primaryCTA}
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {content.objections.items.map((item, i) => (
              <div
                key={i}
                className="rounded-2xl border border-slate-200/70 bg-white p-6 shadow-[0_2px_8px_rgba(0,0,0,0.03)] hover:border-slate-300 transition-colors"
              >
      ...
      >
        ...
        <div className="absolute inset-0 bg-[linear-gradient(to_right,#1e293b_1px,transparent_1px),linear-gradient(to_bottom,#1e293b_1px,transparent_1px)] bg-[size:4rem_4rem] opacity-15 pointer-events-none [mask-image:radial-gradient(ellipse_60%_60%_at_50%_50%,#000_70%,transparent_100%)]" />
        ...
        <div className="max-w-2xl mx-auto px-4 text-center relative z-10">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white mb-5 leading-[1.06] tracking-tight">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              href={PRIMARY_HREF}
              className="w-full sm:w-auto flex items-center justify-center gap-2 text-base font-extrabold bg-blue-600 text-white px-9 py-4 rounded-xl shadow-[0_0_40px_rgba(37,99,235,0.3)] hover:bg-blue-500 transition-all hover:-translate-y-1"
              data-track="cta_primary"
            >
            ...
            <a
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-8 py-4 rounded-xl text-slate-200 font-semibold transition-all text-sm"
              data-track="cta_secondary"
            >
      ...
      {/* Sticky mobile CTA (client, appears after scroll) */}
      <StickyMobileCTA label={content.stickyMobileCTA} href={PRIMARY_HREF} />
    </main>
  )
}
